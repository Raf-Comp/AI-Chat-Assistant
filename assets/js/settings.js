...
(function($) {
    ...
    function initApiTesting() {
        ...
        $('#test-bitbucket-api').on('click', function(e) {
            ...
            console.log('Testowanie połączenia z API Bitbucket...');
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        console.log('Test API Bitbucket zakończony sukcesem');
                    } else {
                        resultContainer.removeClass('loading').addClass('error');
                        resultContainer.html('<span class="dashicons dashicons-no-alt"></span> ' + (response.data && response.data.message ? response.data.message : 'Nie udało się połączyć z API Bitbucket.'));
                        console.error('Błąd testu API Bitbucket:', response.data);
                    }
                },
                error: function(xhr, status, error) {
                    resultContainer.removeClass('loading').addClass('error');
                    resultContainer.html('<span class="dashicons dashicons-no-alt"></span> ' + 'Wystąpił błąd podczas testowania połączenia.');
                    console.error('Błąd AJAX podczas testu API Bitbucket:', error);
                }
            });
        });
    }
    ...
    /**
     * Funkcja pomocnicza do testowania połączenia z API
     */
    function testApiConnection(apiType, apiKeySelector, resultContainerSelector) {
        var button = $(apiType === 'claude' ? '#test-claude-api' : 
                     apiType === 'github' ? '#test-github-api' : 
                     apiType === 'gitlab' ? '#test-gitlab-api' : '');
        ...
        console.log('Testowanie połączenia z API ' + apiType + '...');
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    resultContainer.removeClass('loading').addClass('success');
                    resultContainer.html('<span class="dashicons dashicons-yes-alt"></span> ' + response.data.message);
                    console.log('Test API ' + apiType + ' zakończony sukcesem');
                } else {
                    resultContainer.removeClass('loading').addClass('error');
                    resultContainer.html('<span class="dashicons dashicons-no-alt"></span> ' + (response.data && response.data.message ? response.data.message : 'Nie udało się połączyć z API ' + apiType + '.'));
                    console.error('Błąd testu API ' + apiType + ':', response.data);
                }
            },
            error: function(xhr, status, error) {
                resultContainer.removeClass('loading').addClass('error');
                resultContainer.html('<span class="dashicons dashicons-no-alt"></span> ' + 'Wystąpił błąd podczas testowania połączenia.');
                console.error('Błąd AJAX podczas testu API ' + apiType + ':', error);
            }
        });
    }
